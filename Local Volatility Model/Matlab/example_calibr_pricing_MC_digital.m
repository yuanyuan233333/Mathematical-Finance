...
S = lv_simulation_log(T,Spot,r,q,V,K,N,M,expiry); % hist(S(1,:),100)

% LV price of a digital option
discount_factor = discount(T,r,expiry);
price_lv = discount_factor*mean( (S(1,:) > strike) );

% MC simulation (Black)
time_idx = find(T>=expiry,1);
sigma = interp1(K(:,time_idx),MktVol(:,time_idx),strike);
S_B = black_simulation_log(T,Spot,r,q,sigma,N,expiry);

% Black price of a digital option
price_black = discount_factor*mean( (S_B(1,:) > strike) );
